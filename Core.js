    ...
    body = m.text || "";
        ...
        async function getRealTimeInfo(prompt) {
            try {
                ...
                if (timeRegex.test(text)) { const now = new Date(); return `🕒 *Current Date & Time:* ${now.toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}`; }
                ...
                if (newsRegex.test(text)) {
                    ...
                    const top = news.data.results?.slice(0, 3).map((n, i) => `📰 ${i + 1}. ${n.title}`).join("\n") || "No headlines found.";
                ...
                if (weatherRegex.test(text)) {
                    ...
                    return `🌦️ *Current Weather${city ? " in " + city : ""}:* ${weather.data}`;
                }
                ...
                if (cryptoRegex.test(text)) {
                    ...
                    return `💰 *Bitcoin Price:* $${data.data.bpi.USD.rate}`;
                }
                return null;
            } catch (err) { console.error("🌐 Real-time info fetch error:", err.message); return "⚠️ Sorry, couldn’t fetch live info right now!"; }
        }
        ...
        if (chatbotStatus) {
            ...
            if (m.isGroup && csts == "true" && !icmd && !isCmd) {
                if (m.quoted && m.quoted.sender == botNumber) {
                    ...
                    } catch (err) { console.error("❌ Group Chatbot Error:", err.message); await m.reply("⚠️ Nyaa~ system glitch detected 😅"); }
                }
            }
        }
        ...
        if (PMchatBotStatus) {
            ...
            if (!m.isGroup && PMcsts == "true" && !icmd && !isCmd) {
                ...
                try {
                    ...
                    const reply = chat.choices[0].message.content;
                    setTimeout(() => { m.reply(`${reply}\n\n⚡ Powered by *© ${botName}*\nCreated by *Sten-X*`); }, 1800);
                } catch (err) { console.error("❌ PM Chatbot Error:", err.message); await m.reply("⚠️ Oops~ I need a short break time nya 💫"); }
            }
        }
